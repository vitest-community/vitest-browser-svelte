...
// RELEASE_VERSION=2.2.0 pnpm release
// PUBLISH_DRY_RUN=true PUBLISH_BRANCH=main pnpm publish-ci 2.2.0
...
async function getReleaseTag(version: string, publishBranch: string) {
  ...
  const npmView = await $`npm view vitest-browser-svelte dist-tags --json`
